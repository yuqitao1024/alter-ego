...
func TestCommandHandlerStatusReportsModelAndHistoryCount(t *testing.T) {
	...
	handler := NewCommandHandler(Config{Model: "gpt-test"}, store)
	event := channel.MessageEvent{
		Text:     "/status",
		Platform: "lark",
	...
	for _, part := range []string{"platform: lark", "conversation: direct", "conversation_id: oc_1", "sender_id: ou_1", "model: gpt-test", "history_messages: 2"} {
